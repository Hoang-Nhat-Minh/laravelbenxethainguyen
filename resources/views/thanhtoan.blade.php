...
@section('content')
  <div class="container border text-dark"
    style="margin-top:185px; background-color: rgb(255, 211, 211);border-radius: 25px">
    <div class="m-5 p-2" style="font-size: 20px">
      ...
      <div class="mb-3">
        <label class="form-label">Tổng hóa đơn: {{ $bill->price }} VNĐ</label><br>
        <?php
        $totalAmount = $bill->price;
        $transformer = new \PHPViet\NumberToWords\Transformer();
        $amountInWords = $transformer->toWords($totalAmount);
        $amountInWords = ucwords($amountInWords);
        ?>
        <label class="form-label">Bằng chữ: {{ $amountInWords }} VNĐ</label>
      </div>
    </div>
  </div>

  <div class="container mt-5 p-3 text-center bg-dark rounded">
    <h1 class="text-light mt-3 mb-5">QR thanh toán</h1>
    <img src="{{ asset('img/qr.jpg') }}">
    <h2 class="text-light mt-5 mb-3">Cảm ơn quý khách đã tin tưởng và dùng dịch vụ chúng tôi!</h2>
  </div>
@endsection
